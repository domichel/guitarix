...
"""
    ...
    def generate_faust_table(self, p, y, filename=None):
        n = 'dkbuild/%s_table.lib' % self.model
        if filename:
            n = filename
        #y = p(self.signal())
        c = 0;
        ...
        for i in range(len(y)):
            ...
            if c == 2:
                ...
                sys.stdout.write('    low = %g;\n' % (y[0] -aj))
                sys.stdout.write('    high =%g;\n' % ((y[len(self.sig)-1] -aj )/self.divider))
                sys.stdout.write('    step =%g;\n' % ((len(self.sig)-1)*self.operator))
                sys.stdout.write('    size =%d;\n\n' % (len(self.sig)))
                sys.stdout.write('    %s_table = waveform {' % self.model)
                for item in y:
                    if z % 5 == 0:
                        sys.stdout.write(s+"\n    " )
                        s = ""
